 ...
 * @author Eirik Kolås
 * @brief Dynamic model interface. Based on "Fundamentals of Sensor Fusion" by
 * Edmund Brekke
 * @version 0.1
 * @date 2023-10-26
 */
 ...
 * @note To derive from this class, you must override the following virtual
 * functions:
 * @note - f_d
 * @note - Q_d
 */
template <size_t n_dim_x, size_t n_dim_u = n_dim_x, size_t n_dim_v = n_dim_x>
class DynamicModel {
   public:
     ...
     */
    virtual T::Vec_x f_d(double dt,
                         const T::Vec_x& x,
                         const T::Vec_u& u,
                         const T::Vec_v& v) const = 0;
...
/** Linear Time Variant Dynamic Model Interface. [x_k+1 = f_d = A_k*x_k +
 * B_k*u_k + G_k*v_k]
 * @tparam n_dim_x  State dimension
 ...
 */
template <size_t n_dim_x, size_t n_dim_u = n_dim_x, size_t n_dim_v = n_dim_x>
class DynamicModelLTV : public DynamicModel<n_dim_x, n_dim_u, n_dim_v> {
   public:
     ...
     * @note - G_d (optional if n_dim_x == n_dim_v)
     */
    DynamicModelLTV() : DynamicModel<N_DIM_x, N_DIM_u, N_DIM_v>() {}
    ...
    virtual T::Vec_x f_d(double dt,
                         ...
                         const T::Vec_v& v = T::Vec_v::Zero()) const override {
     ...
     * @return T::Vec_x
     */
    auto pred_from_est(double dt,
                       const auto& x_est,
                       const T::Vec_u& u = T::Vec_u::Zero()) const
        -> std::remove_reference_t<decltype(x_est)>
     ...
     * @return T::Vec_x
     */
    T::Gauss_x pred_from_state(double dt,
                               const T::Vec_x& x,
                               const T::Vec_u& u = T::Vec_u::Zero()) const {
   ...
   protected:
    /** Process noise covariance matrix. For expressing the process noise in the
     * state space.
     * @param dt Time step
     * @param x T::Vec_x State
...
/** Continuous Time Linear Time Varying Dynamic Model Interface. It uses exact
 * discretization for everything. So it might be slow. [x_dot = A_c*x + B_c*u +
 * G_c*v]
 * @tparam n_dim_x  State dimension
 * @tparam n_dim_u  Input dimension (Default: n_dim_x)
 * @tparam n_dim_v  Process noise dimension (Default: n_dim_x)
 * @note See https://en.wikipedia.org/wiki/Discretization for more info on how
...
class DynamicModelCTLTV : public DynamicModelLTV<n_dim_x, n_dim_u, n_dim_v> {
   public:
    ...
    /** Continuous Time Linear Time Varying Dynamic Model Interface. [x_dot =
     * A_c*x + B_c*u + G_c*v]
     * @tparam n_dim_x  State dimension
     * @tparam n_dim_u  Input dimension (Default: n_dim_x)
     * @tparam n_dim_v  Process noise dimension (Default: n_dim_x)
     ...
     */
    T::Vec_x f_c(const T::Vec_x& x,
                 const T::Vec_u& u = T::Vec_u::Zero(),
                 const T::Vec_v& v = T::Vec_v::Zero()) const {
    ...
    /** System matrix (Jacobian of continuous time dynamics with respect to
     * state)
     * @param x T::Vec_x State
     * @return State_jac
     */
    ...
    /** Process noise transition matrix. For expressing the process noise in the
     * state space.
     * @param x T::Vec_x State
     * @return Process_noise_jac
     */
    ...
    /** Process noise covariance matrix. This has the same dimension as the
     * process noise.
     * @param x T::Vec_x State
     */
    virtual T::Mat_vv Q_c(const T::Vec_x& x) const = 0;

    /** System dynamics (Jacobian of discrete time dynamics w.r.t. state). Using
    ...
    /** Input matrix (Jacobian of discrete time dynamics w.r.t. input). Using
     * exact discretization.
     * @param dt Time step
     * @param x T::Vec_x
     * @return Input_jac
    ...
    T::Mat_xu B_d(double dt, const T::Vec_x& x) const override {
        ...
        typename T::Mat_xu B_d =
            van_loan.template block<N_DIM_x, N_DIM_u>(0, N_DIM_x);
        return B_d;
    }
    ...
    T::Mat_xv G_d(double dt, const T::Vec_x& x) const override {
        ...
        van_loan.template bottomLeftCorner<N_DIM_v, N_DIM_x>() =
            T::Mat_vx::Zero();
        van_loan.template bottomRightCorner<N_DIM_v, N_DIM_v>() =
            T::Mat_vv::Zero();
        ...
        // T::Mat_xx A_d = van_loan.template block<N_DIM_x, N_DIM_x>(0, 0);
        typename T::Mat_xv G_d =
            van_loan.template block<N_DIM_x, N_DIM_v>(0, N_DIM_x);
        return G_d;
    }

    /** Discrete time process noise covariance matrix. This is super scuffed,
     * but it works... (As long as G_d^T*G_d is invertible) Overriding
     * DynamicModel::Q_d
     * @param dt Time step
     * @param x T::Vec_x
    ...
    T::Mat_vv Q_d(double dt, const T::Vec_x& x) const override {
        ...
        typename T::Mat_xv G_d = this->G_d(dt, x);
        // pseudo inverse of G_d
        typename T::Mat_vx G_d_pinv =
            G_d.completeOrthogonalDecomposition().pseudoInverse();
    ...
    T::Mat_xx GQGT_d(double dt, const T::Vec_x& x) const override {
        ...
        van_loan.template topRightCorner<N_DIM_x, N_DIM_x>() =
            G_c * Q_c * G_c.transpose();
        van_loan.template bottomLeftCorner<N_DIM_x, N_DIM_x>() =
            T::Mat_xx::Zero();
        van_loan.template bottomRightCorner<N_DIM_x, N_DIM_x>() =
            A_c.transpose();

        van_loan *= dt;
        van_loan = van_loan.exp();

        typename T::Mat_xx A_d =
            van_loan.template block<N_DIM_x, N_DIM_x>(N_DIM_x, N_DIM_x)
                .transpose();
        typename T::Mat_xx A_d_inv_GQGT_d =
            van_loan.template block<N_DIM_x, N_DIM_x>(
                0, N_DIM_x);  // A_d^(-1) * G * Q * G^T
