...
constexpr std::array<bool, N> matching_state_names(
    const std::array<T, N>& array1,
    const std::array<T, M>& array2) {
 ...
 */
template <vortex::concepts::DynamicModelWithDefinedSizes... DynModels>
class ImmModel {
   public:
    ...
    static constexpr StateNames ALL_STATE_NAMES = {
        {DynModels::StateT::STATE_NAMES}...};

    using DynModTuple = std::tuple<DynModels...>;
    using GaussTuple_x =
        std::tuple<typename Types_x<DynModels::N_DIM_x>::Gauss_x...>;
    using Vec_n = Eigen::Vector<double, N_MODELS>;
    using Mat_nn = Eigen::Matrix<double, N_MODELS, N_MODELS>;

    template <size_t i>
    using DynModT = typename std::tuple_element<i, DynModTuple>::type;

    template <size_t i>
    using State = DynModT<i>::State;

    template <size_t i>
    using T = Types_xuv<N_DIMS_x[i], N_DIMS_u[i], N_DIMS_v[i]>;

    /**
     * @brief Construct a new ImmModel object
     * @tparam DynModels Dynamic models to use.
     * @param jump_matrix Markov jump chain matrix for the transition
     * probabilities. I.e. the probability of switching from model i to model j
     * is `jump_matrix(i,j)`. Diagonal should be 0.
     * @param hold_times Expected holding time in seconds for each state.
     ...
     * @note - The jump matrix specifies the probability of switching to a model
     * WHEN a switch occurs.
     * @note - The holding times specifies HOW LONG a state is expected to be
     * held between switches.
     * @note - In order to change the properties of a model, you must get the
     * model using `get_model<i>()`
     */
    ImmModel(Mat_nn jump_matrix, Vec_n hold_times, DynModels... models)
        ...
        for (int i = 0; i < jump_matrix_.rows(); i++) {
            if (jump_matrix_.row(i).sum() != 1.0) {
                throw std::invalid_argument("Jump matrix row " +
                                            std::to_string(i) +
                                            " should sum to 1");
            }
        }
    ...
    Mat_nn get_pi_mat_d(double dt) const {
        // Cache the pi matrix for the same time step as it is likely to be
        // reused and is expensive to compute
     ...
     */
    template <size_t i>
    const DynModT<i>& get_model() const {
     ...
     */
    template <size_t i>
    T<i>::Vec_x f_d(double dt,
                    const T<i>::Vec_x& x,
                    const T<i>::Vec_u& u = T<i>::Vec_u::Zero(),
                    const T<i>::Vec_v& v = T<i>::Vec_v::Zero()) const {
     ...
     */
    template <size_t i>
    T<i>::Mat_vv Q_d(double dt, const T<i>::Vec_x& x) const {
        return get_model<i>().Q_d(dt, x);
    }

    static constexpr int N_DIM_x(size_t model_index) {
        return N_DIMS_x.at(model_index);
    }
    static constexpr int N_DIM_u(size_t model_index) {
        return N_DIMS_u.at(model_index);
    }
    static constexpr int N_DIM_v(size_t model_index) {
        return N_DIMS_v.at(model_index);
    }
...
/**
 * @brief Class for resizing the state vector of a sensor model to fit with
 * multiple dynamic models.
 *
 * @tparam n_dim_a The dimension of the desired state vector.
 * @tparam SensModT The sensor model to use.
 */
template <size_t n_dim_a,
          vortex::concepts::SensorModelWithDefinedSizes SensModT>
class ImmSensorModel {
   public:
     ...
     *
     * @param sensor_model The sensor model to use. Must have a copy
     * constructor.
     */
    ImmSensorModel(SensModT sensor_model) : sensor_model_(sensor_model) {
        static_assert(N_DIM_a >= SensModT::SensModI::N_DIM_x,
                      "N_DIM_a must be greater than or equal to the state "
                      "dimension of the sensor model");
    }

    T::Vec_z h(const T::Vec_a& x, const T::Vec_w& w) const {
        return sensor_model_.h(x.template head<N_DIM_x>(), w);
    }

    T::Mat_ww R(const T::Vec_x& x) const {
 ...
 */
template <size_t n_dim_a,
          vortex::concepts::SensorModelLTVWithDefinedSizes SensModT>
class ImmSensorModelLTV {
   public:
     ...
     * constructor.
     */
    ImmSensorModelLTV(SensModT sensor_model) : sensor_model_(sensor_model) {
        static_assert(N_DIM_a >= N_DIM_x,
                      "N_DIM_a must be greater than or equal to the state "
                      "dimension of the sensor model");
    }
    ...
    T::Mat_za C(const T::Vec_a& x) const {
        ...
        C_a.template leftCols<N_DIM_x>() =
            sensor_model_.C(x.template head<N_DIM_x>());
        return C_a;
    }

    T::Mat_zw H(const T::Vec_a& x) const {
        return sensor_model_.H(x.template head<N_DIM_x>());
    }

    T::Mat_ww R(const T::Vec_a& x) const {
        return sensor_model_.R(x.template head<N_DIM_x>());
    }

    T::Gauss_z pred_from_est(const T::Gauss_a& x_est) const {
        typename T::Vec_x mean = x_est.mean().template head<N_DIM_x>();
        typename T::Mat_xx cov =
            x_est.cov().template topLeftCorner<N_DIM_x, N_DIM_x>();
        return sensor_model_.pred_from_est(typename T::Gauss_x{mean, cov});
    }

    T::Gauss_z pred_from_state(const T::Vec_a& x) const {
