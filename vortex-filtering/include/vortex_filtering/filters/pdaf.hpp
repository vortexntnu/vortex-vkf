...
template <class DynModT, class SensModT> class PDAF {
public:
  ...
  PDAF() { std::cout << "Created PDAF class!" << std::endl; }

  std::tuple<Gauss_x, MeasurementsZd, MeasurementsZd, Gauss_x, Gauss_z, StatesXd> predict_next_state(const Gauss_x &x_est, const MeasurementsZd &z_meas,
                                                                                                     double timestep, const DynModPtr &dyn_model,
                                                                                                     const SensModPtr &sen_model, double gate_threshold,
                                                                                                     double prob_of_detection, double clutter_intensity) const
  {
  ...
  std::tuple<MeasurementsZd, MeasurementsZd> apply_gate(const MeasurementsZd &z_meas, const Gauss_z &z_pred, double gate_threshold) const
  {
  ...
  Eigen::VectorXd get_weights(const MeasurementsZd &z_meas, const Gauss_z &z_pred, double prob_of_detection, double clutter_intensity) const
  {
