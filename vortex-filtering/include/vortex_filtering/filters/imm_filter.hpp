...
template <typename ImmModelT, class SensModT> class ImmFilter {
public:
	...
	ImmFilter(const ImmModelT &imm_model, const SensModT &sensor_model) : imm_model_(imm_model), sensor_model_(sensor_model)
	{
	 ...
	 * @param dt Time step
	 */
	std::tuple < std::vector<prob::MultiVarGauss<N_DIM_x>>, std::vector<prob::MultiVarGauss<N_DIM_x>>,
	    std::vector < prob::MultiVarGauss < N_DIM_z >>>>
	        mode_match_filter(const std::vector<prob::MultiVarGauss<N_DIM_x>> &moment_based_preds, Vec_zz z_meas, double dt)
	{

		std::tuple < std::vector<prob::MultiVarGauss<N_DIM_x>>, std::vector<prob::MultiVarGauss<N_DIM_x>>,
		    std::vector < prob::MultiVarGauss < N_DIM_z >>>> ekf_outs;
	 ...
	 * @param dt Time step
	 */
	std::tuple < prob::GaussianMixture<N_DIM_x>, prob::GaussianMixture<N_DIM_x>,
	    prob::GaussianMixture<N_DIM_z> step(const prob::GaussianMixture<N_DIM_x> &x_est_prev, Vec_z z_meas, double dt)
	{
