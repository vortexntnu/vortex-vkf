...
public:
  ...
  ImmFilter(ImmModelT imm_model, SensModTPtr sensor_model)
	: imm_model_(std::move(imm_model)), sensor_model_(std::move(sensor_model))
  {
		...
		[](auto&&... n_dim_x) {
		  if (!((N_DIM_x == n_dim_x) && ...))
		  {
			throw std::runtime_error(
				"Number of states in dynamic models does not match the number of states in the sensor model.");
		  }
		},
   ...
   */
  std::array<std::tuple<Gauss_x, Gauss_x, Gauss_z>, N_MODELS>
  mode_matched_filter(const std::array<Gauss_x, N_MODELS>& moment_based_preds, const Vec_z& z_meas, double dt)
  {
	...
	std::apply(
		[this, &ekf_outs, &moment_based_preds, &z_meas, &dt](auto&&... modelPtr) {
		  size_t i = 0;
		  (((ekf_outs[i] = filter::EKF_M<decltype(*modelPtr), SensModT>::step(modelPtr, sensor_model_,
																			  moment_based_preds[i], z_meas, dt)),
			i++),
		   ...);
		},
