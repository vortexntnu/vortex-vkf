...
// Concept to ensure that the assignment algorithm is compatible with the cost matrix
template <typename T>
concept assignment_algorithm = requires(T a, const Eigen::MatrixXd &cost_matrix) {
  {
    a(cost_matrix)
  } -> std::same_as<std::pair<double, Eigen::VectorXi>>;
};
