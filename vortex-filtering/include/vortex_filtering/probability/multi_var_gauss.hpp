   ...
   public:
    ...
    double pdf(const Vec_n& x) const {
        ...
        return std::exp(exponent) /
               std::sqrt(std::pow(2 * std::numbers::pi, size()) *
                         cov().determinant());
    }
    ...
    double logpdf(const Vec_n& x) const {
        ...
        double exponent = -0.5 * diff.transpose() * cov_inv() * diff;
        return exponent -
               0.5 * std::log(std::pow(2 * std::numbers::pi, size()) *
                              cov().determinant());
    }
    ...
    /** Construct a Standard Gaussian Distribution with zero mean and identity
     * covariance matrix
     * @return MultiVarGauss
     */
    static MultiVarGauss Standard() {
        return {Vec_n::Zero(), Mat_nn::Identity()};
    }
