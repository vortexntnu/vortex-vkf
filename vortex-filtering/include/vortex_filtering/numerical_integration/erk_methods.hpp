...
class Forward_Euler {
   public:
    ...
    static Vec_x integrate(Dyn_mod_func f,
                           double dt,
                           const Vec_x& x0,
                           double t0 = 0.0) {
        Vec_x k1 = f(t0, x0);
...
using Forward_Euler_M = Forward_Euler<DynModT::N_DIM_x>;
...
class RK4 {
   public:
     ...
     *
     * @param f Function to integrate, must be of the form Vec_x(double dt,
     * const Vec_x&x_k, double t_k=0)
     * @param dt Time step
...
template <typename DynModT>
using RK4_M = RK4<DynModT::N_DIM_x>;
 ...
 */
template <int n_dim_x, int n_stages>
class Butcher {
   public:
    using Vec_x = Eigen::Vector<double, n_dim_x>;
    using Dyn_mod_func = std::function<Vec_x(double t0, const Vec_x& x0)>;
    ...
    /**
     * @brief Construct an ERK method from a Butcher table. Cannot be passed as
     * a class to a template as it needs the non-static members A, b and c to be
     * initialized in an object.
     * @param A
     * @param b
     * @param c
     */
    Butcher(Mat_nn A, Vec_n b, Vec_n c) : _A{A}, _b{b}, _c{c} {
        ...
        if (A.isLowerTriangular() == false) {
            throw std::invalid_argument(
                "Butcher table: A is not lower triangular");
        }
        ...
        for (int i = 0; i < A.rows(); i++) {
            if (std::abs(A.row(i).sum() - c(i)) > 1e-6) {
                throw std::invalid_argument(
                    "Butcher table: row " + std::to_string(i) +
                    " in A does not sum to c(" + std::to_string(i) + ")");
            }
        }
    }

    Vec_x integrate(Dyn_mod_func f,
                    double dt,
                    const Vec_x& x0,
                    double t0 = 0.0) {
...
class ODE45 {
   public:
     ...
     * @param min_step_size Minimum valid value for the step size in the
     * integration
     */
    ODE45(double abs_error = 1e-6,
          double rel_error = 1e-6,
          size_t max_iterations = 1000,
          double min_step_size = 1e-9)
        : ODE45(Vec_x::Ones() * abs_error,
                Vec_x::Ones() * rel_error,
                max_iterations,
                min_step_size) {}
     ...
     * @brief Variable step size Runge-Kutta method
     *
     * @param abs_error_vec absolute error vector. Specify a value for each
     * state variable
     * @param rel_error_vec relative error vector. Specify a value for each
     * state variable
     * @param max_iterations Maximum number of iterations before giving up
     * @param min_step_size Minimum valid value for the step size in the
     * integration
     */
    ODE45(Vec_x abs_error_vec = Vec_x::Ones() * 1e-6,
          Vec_x rel_error_vec = Vec_x::Ones() * 1e-6,
          size_t max_iterations = 1000,
          double min_step_size = 1e-9)
        : _Atol{abs_error_vec},
          _Rtol{rel_error_vec},
    ...
    Vec_x integrate(Dyn_mod_func f,
        ...
        for (size_t iter = 0; iter < _max_iter; iter++) {
            ...
            for (size_t i = 0; i < n_stages; i++) {
                k.col(i) =
                    f(t_i + _c(i) * h, x_i + h * k * _A.row(i).transpose());
            }
            ...
            double h_new;
            static constexpr double f_ac =
                std::pow(0.25, 1.0 / (q + 1));  // safety factor
            h_new = f_ac * h *
                    std::pow(1 / error, 1.0 / (q + 1));  // optimal step size
            h_new = std::max(h_new, 0.2 * h);  // limit step size decrease
            h_new = std::min(h_new, 5.0 * h);  // limit step size increase
            h_new = std::min(
                h_new, t0pdt - t_i);  // limit step size to not overshoot t_kp1
            h = h_new;

            if (h < _min_step_size) {
                throw std::runtime_error(
                    "Could not reach tolerance within minimum step size" +
                    std::to_string(_min_step_size));
            }
        }
        throw std::runtime_error(
            "Could not reach tolerance within maximum number of iterations " +
            std::to_string(_max_iter));
    }
