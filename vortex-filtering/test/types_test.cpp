...
TEST(Types, x_2_z_1) {
    ...
    static_assert(std::is_same<decltype(x), Eigen::Vector2d>::value,
                  "x is not a Vector2d");
    static_assert(std::is_same<decltype(z), Eigen::Vector<double, 1>>::value,
                  "z is not a Vector1d");
...
TEST(Concepts, MatConvertibleTo) {
    ...
    static_assert(
        vortex::concepts::mat_convertible_to<Eigen::Matrix3d, Eigen::Matrix3d>);
    static_assert(!vortex::concepts::mat_convertible_to<Eigen::Matrix3d,
                                                        Eigen::Matrix2d>);
...
TEST(Concepts, MultiVarGaussLike) {
    ...
    using StateT =
        vortex::State<S, S::position, S::position, S::velocity, S::velocity>;
    static_assert(
        vortex::concepts::MultiVarGaussLike<StateT, StateT::N_STATES>);
...
TEST(Concepts, Models) {
    ...
    using DynMod =
        vortex::models::interface::DynamicModel<N_DIM_x, N_DIM_u, N_DIM_v>;
    using DynModLTV =
        vortex::models::interface::DynamicModelLTV<N_DIM_x, N_DIM_u, N_DIM_v>;
    using SensMod =
        vortex::models::interface::SensorModel<N_DIM_x, N_DIM_z, N_DIM_w>;
    using SensModLTV =
        vortex::models::interface::SensorModelLTV<N_DIM_x, N_DIM_z, N_DIM_w>;

    static_assert(
        vortex::concepts::DynamicModel<DynMod, N_DIM_x, N_DIM_u, N_DIM_v>);
    static_assert(vortex::concepts::DynamicModelLTV<DynModLTV, N_DIM_x, N_DIM_u,
                                                    N_DIM_v>);
    static_assert(
        vortex::concepts::SensorModel<SensMod, N_DIM_x, N_DIM_z, N_DIM_w>);
    static_assert(vortex::concepts::SensorModelLTV<SensModLTV, N_DIM_x, N_DIM_z,
                                                   N_DIM_w>);
