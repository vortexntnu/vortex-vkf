...
class EKFTestCVModel : public ::testing::Test {
   protected:
    ...
    EKFTestCVModel() : dynamic_model_(1e-2), sensor_model_(1e-2) {}
    CVModel dynamic_model_;
    PosMeasModel sensor_model_;
};
...
TEST_F(EKFTestCVModel, predict) {
    ...
    auto [x_est_pred, z_est_pred] =
        EKF::predict(dynamic_model_, sensor_model_, dt, x);
...
TEST_F(EKFTestCVModel, convergence) {
    ...
    for (int i = 0; i < 100; i++) {
        // Simulate
        Vec_x x_true_i =
            dynamic_model_.sample_f_d(dt, x_true.back(), Vec_u::Zero(), gen);
        Vec_z z_meas_i = sensor_model_.sample_h(x_true_i, gen);
        x_true.push_back(x_true_i);
        z_meas.push_back(z_meas_i);

        // Predict and update
        auto [x_est_upd, x_est_pred, z_est_pred] = EKF::step(
            dynamic_model_, sensor_model_, dt, x_est.back(), z_meas_i);
    ...
    // Plot the results
    std::vector<double> x_true_x, x_true_y, x_true_u, x_true_v, x_est_x,
        x_est_y, x_est_u, x_est_v, z_meas_x, z_meas_y;
    ...
    gp << "plot '-' with lines title 'True', '-' with lines title 'Estimate', "
          "'-' with points title 'Measurements' ps 2\n";  // Modified to make
                                                          // dots larger
