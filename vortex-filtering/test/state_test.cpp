...
TEST(State, typeChecks) {
    ...
    using S = StateName;
    using StateT =
        State<S, S::position, S::velocity, S::velocity, S::acceleration>;
...
TEST(State, getMean) {
    ...
    ASSERT_TRUE(isApproxEqual(state.mean_of<S::position>(),
                              x.mean().template head<1>(), 1e-6));
    ASSERT_TRUE(isApproxEqual(state.mean_of<S::velocity>(),
                              x.mean().template segment<2>(1), 1e-6));
    ASSERT_TRUE(isApproxEqual(state.mean_of<S::acceleration>(),
                              x.mean().template tail<1>(), 1e-6));
}
...
TEST(State, getCov) {
    ...
    ASSERT_TRUE(isApproxEqual(state.cov_of<S::position>(),
                              x.cov().template block<1, 1>(0, 0), 1e-6));
    ASSERT_TRUE(isApproxEqual(state.cov_of<S::velocity>(),
                              x.cov().template block<2, 2>(1, 1), 1e-6));
    ASSERT_TRUE(isApproxEqual(state.cov_of<S::acceleration>(),
                              x.cov().template block<1, 1>(3, 3), 1e-6));
}
...
TEST(State, setCov) {
    ...
    ASSERT_TRUE(isApproxEqual(state.cov_of<S::position>(),
                              x_new.cov().template block<1, 1>(0, 0), 1e-6));
    ASSERT_TRUE(isApproxEqual(state.cov_of<S::velocity>(),
                              x_new.cov().template block<2, 2>(1, 1), 1e-6));
    ASSERT_TRUE(isApproxEqual(state.cov_of<S::acceleration>(),
                              x_new.cov().template block<1, 1>(3, 3), 1e-6));
