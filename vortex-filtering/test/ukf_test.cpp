...
TEST_F(UKFtest, Predict) {
    ...
    auto [x_est_pred, z_est_pred] =
        UKF::predict(dynamic_model_, sensor_model_, dt, x);
...
TEST_F(UKFtest, Convergence) {
    ...
    for (int i = 0; i < n_steps - 1; i++) {
        ...
        w << d_noise(gen);
        Vec_x x_true_i =
            dynamic_model_.f_d(dt, x_true.back(), Vec_x::Zero(), v);
        Vec_z z_meas_i = sensor_model_.h(x_true_i) + w;
        x_true.push_back(x_true_i);
        z_meas.push_back(z_meas_i);

        // Predict and update
        auto [x_est_upd, x_est_pred, z_est_pred] =
            UKF::step(dynamic_model_, sensor_model_, dt, x_est.back(), z_meas_i,
                      Vec_x::Zero());
    ...
    for (int i = 0; i < n_steps; i++) {
        ...
        x_p_std.push_back(x_est.at(i).mean()(0) +
                          std::sqrt(x_est.at(i).cov()(0, 0)));  // x_est + std
        x_m_std.push_back(x_est.at(i).mean()(0) -
                          std::sqrt(x_est.at(i).cov()(0, 0)));  // x_est - std
    }
    ...
    gp << "set grid\n";
    gp << "set key left top\n";
    gp << "plot '-' using 1:($2-$3):($2+$3) with filledcurves title 'x_{est} "
          "+/- std' fs solid 0.5, "
