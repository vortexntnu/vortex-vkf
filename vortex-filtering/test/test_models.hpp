...
class SimpleDynamicModel : public DynamicModelCTLTV<2> {
   public:
    ...
    T::Mat_xx A_c(const T::Vec_x& = T::Vec_x::Zero()) const override {
        return -T::Mat_xx::Identity();
    }

    T::Mat_vv Q_c(const T::Vec_x& = T::Vec_x::Zero()) const override {
        return T::Mat_xx::Identity();
    }
};
...
class NonlinearModel1 : public DynamicModel<1, 1, 1> {
   public:
    ...
    T::Mat_vv Q_d(double = 0.0,
                  const T::Vec_x& = T::Vec_x::Zero()) const override {
        return T::Mat_xx::Identity() * cov_;
    }
...
class LorenzAttractorCT : public DynamicModel<3> {
   public:
    ...
    LorenzAttractorCT(double std_dev)
        : cov_(std_dev * std_dev), sigma_(10.0), beta_(8.0 / 3.0), rho_(28.0) {}

    T::Vec_x f_c(const T::Vec_x& x,
                 const T::Vec_u& = T::Vec_u::Zero(),
                 const T::Vec_v& v = T::Vec_v::Zero()) const {
        typename T::Vec_x x_next;
        x_next << sigma_ * (x(1) - x(0)), x(0) * (rho_ - x(2)) - x(1),
            x(0) * x(1) - beta_ * x(2);
        x_next += v;
        return x_next;
    }

    T::Vec_x f_d(double dt,
                 const T::Vec_x& x,
                 const T::Vec_u& u = T::Vec_u::Zero(),
                 const T::Vec_v& v = T::Vec_v::Zero()) const override {
        using Dyn_mod_func =
            std::function<T::Vec_x(double t, const T::Vec_x& x)>;

        Dyn_mod_func f_c = [this, &u, &v](double, const T::Vec_x& x) {
            return this->f_c(x, u, v);
        };
    ...
    T::Mat_vv Q_d(double dt,
                  const T::Vec_x& = T::Vec_x::Zero()) const override {
        return T::Mat_xx::Identity() * cov_ * dt;
    }
...
class LorenzAttractorCTLTV
    : public vortex::models::interface::DynamicModelCTLTV<3> {
   public:
    ...
    LorenzAttractorCTLTV(double std_dev)
        : cov_(std_dev * std_dev), sigma_(10.0), beta_(8.0 / 3.0), rho_(28.0) {}
