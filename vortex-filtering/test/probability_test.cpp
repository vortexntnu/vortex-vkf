...
TEST(MultiVarGauss, initGaussian) {
    vortex::prob::MultiVarGauss<2> gaussian(Eigen::Vector2d::Zero(),
...
TEST(MultiVarGauss, assignmentOperator) {
    vortex::prob::MultiVarGauss<2> gaussian1(Eigen::Vector2d::Zero(),
                                             Eigen::Matrix2d::Identity());
    vortex::prob::MultiVarGauss<2> gaussian2(Eigen::Vector2d::Ones(),
                                             Eigen::Matrix2d::Identity());
...
TEST(MultiVarGauss, pdf) {
    ...
    EXPECT_NEAR(gaussian.pdf(Eigen::Vector2d{1, 0}),
                1 / (2 * std::sqrt(M_E) * PI), 1e-15);
    EXPECT_NEAR(gaussian.pdf(Eigen::Vector2d{0, 1}),
                1 / (2 * std::sqrt(M_E) * PI), 1e-15);
    EXPECT_NEAR(gaussian.pdf(Eigen::Vector2d{1, 1}), 1 / (2 * M_E * PI), 1e-15);

    gaussian = vortex::prob::MultiVarGauss<2>(Eigen::Vector2d{0, 0},
                                              Eigen::Matrix2d{{2, 1}, {1, 2}});

    EXPECT_NEAR(gaussian.pdf(Eigen::Vector2d{0, 0}),
                1 / (2 * std::sqrt(3) * PI), 1e-15);
    EXPECT_NEAR(gaussian.pdf(Eigen::Vector2d{1, 0}),
                1 / (2 * std::sqrt(3) * std::exp(1.0 / 3) * PI), 1e-15);
    EXPECT_NEAR(gaussian.pdf(Eigen::Vector2d{0, 1}),
                1 / (2 * std::sqrt(3) * std::exp(1.0 / 3) * PI), 1e-15);
    EXPECT_NEAR(gaussian.pdf(Eigen::Vector2d{1, 1}),
                1 / (2 * std::sqrt(3) * std::exp(1.0 / 3) * PI), 1e-15);
}

TEST(MultiVarGauss, invalidCovariance) {
    EXPECT_THROW(vortex::prob::MultiVarGauss<2>(Eigen::Vector2d::Zero(),
                                                Eigen::Matrix2d::Zero()),
                 std::invalid_argument);
    EXPECT_THROW(vortex::prob::MultiVarGauss<2>(
                     Eigen::Vector2d::Zero(), Eigen::Matrix2d{{1, 0}, {0, 0}}),
                 std::invalid_argument);
    EXPECT_THROW(
        vortex::prob::MultiVarGauss<2>(Eigen::Vector2d::Zero(),
...
TEST(MultiVarGauss, sample) {
    ...
    gp << "plot '-' with circles title 'Samples' fs transparent solid 0.05 "
          "noborder\n";
    gp.send1d(samples);

    gp << "set object 1 ellipse center " << true_mean(0) << "," << true_mean(1)
       << " size " << majorAxisLength << "," << minorAxisLength << " angle "
       << angle << "fs empty border lc rgb 'cyan'\n";
    gp << "replot\n";
#endif
...
TEST(isContainerConcept, compileTimeChecks) {
    static_assert(
        vortex::prob::concepts::is_container<std::vector<double>, double>);
    static_assert(
        vortex::prob::concepts::is_container<std::array<double, 4>, double>);
    static_assert(
        vortex::prob::concepts::is_container<std::vector<vortex::prob::Gauss2d>,
                                             vortex::prob::Gauss2d>);
    static_assert(
        vortex::prob::concepts::is_container<Eigen::Vector2d, double>);
    static_assert(
        vortex::prob::concepts::is_container<Eigen::VectorXd, double>);
    static_assert(
        vortex::prob::concepts::is_container<Eigen::RowVectorXd, double>);
