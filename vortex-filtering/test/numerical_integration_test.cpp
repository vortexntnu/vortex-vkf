...
class NumericalIntegration : public ::testing::Test {
   protected:
    ...
    void run_iterations(Dyn_mod_func f,
                        Dyn_mod_func f_exact,
                        size_t num_iters,
                        double tolerance) {
        ...
        std::cout << "Error: " << x_est.back()(0) - x_exact.back()(0)
                  << std::endl;
    ...
    void plot_result(std::string title = "ERK convergence") {
        ...
        gp << "set grid\n";
        gp << "set key left top\n";
        gp << "plot '-' with lines title 'True', '-' with lines title "
              "'Approx'\n";
        gp.send1d(std::make_tuple(
            t, vortex::plotting::extract_state_series(x_exact, 0)));
        gp.send1d(std::make_tuple(
            t, vortex::plotting::extract_state_series(x_est, 0)));
#endif
        (void)title;
    }
};
...
TEST_F(NumericalIntegration, RK4sinFunc) {
    ...
    run_iterations<vortex::integrator::RK4<N_DIM_x>>(sin_func, sin_func_exact,
                                                     n_iters, 1e-4);
    // plot_result();
}
...
TEST_F(NumericalIntegration, EulerSinFunc) {
    ...
    run_iterations<vortex::integrator::Forward_Euler<N_DIM_x>>(
        sin_func, sin_func_exact, n_iters, 1e-1);
...
TEST_F(NumericalIntegration, ButcherMidpointSinFunc) {
    ...
    auto midpoint =
        std::make_shared<vortex::integrator::Butcher<N_DIM_x, n_stages>>(A, b,
                                                                         c);
    size_t n_iters = 5000;
    ...
    for (size_t i = 0; i < num_iters; i++) {
        x_est.push_back(
            midpoint->integrate(sin_func, dt, x_est.at(i), t.at(i)));
        t.push_back(t.at(i) + dt);
...
TEST_F(NumericalIntegration, ODE45sinFuncRealTime) {
    ...
    init(0, x0);
    auto rk45 =
        std::make_shared<vortex::integrator::ODE45<N_DIM_x>>(1e-20, 1e-20);
    size_t n_iters = 5000;
...
TEST_F(NumericalIntegration, ODE45sinFuncLongStep) {
    ...
    x0 << 1;
    init(0, x0);
    auto rk45 = std::make_shared<vortex::integrator::ODE45<N_DIM_x>>(
        1e-20, 1e-20, 100000);
    dt = 5;
    size_t n_iters = 1;
